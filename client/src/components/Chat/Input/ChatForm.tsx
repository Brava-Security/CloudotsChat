...
const ChatForm = ({ index = 0 }) => {
  ...
  const [suggestions, setSuggestions] = useState<string[]>([
    "Write a SQL detection for AWS 'PutBucketEncryption'",
    "Explain 'CreateServiceAccount' GCP audit log",
    "How to simulate 'AuthorizeSecurityGroupIngress'?",
    "Provide the cloudtrail 'ModifyInstanceAttribute' event structure"
  ]);
  ...
  const handleSuggestionClick = (suggestion: string) => {
    if (textAreaRef.current) {
      ...
      methods.setValue('text', suggestion, { shouldValidate: true });
      textAreaRef.current.focus();
    }
  };
  ...
  useEffect(() => {
    ...
    const isNewChat = conversation?.conversationId === 'new';
    console.log('Suggestions check rerender:', !messages || messages.length === 0, isNewChat);
    setShowSuggestions((!messages || messages.length === 0) || isNewChat);
